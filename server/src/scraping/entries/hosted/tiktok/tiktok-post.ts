...
import { TiktokPostMetadata } from "src/scraping/options/hosted/tiktok/utils/interfaces/tiktok-post-metadata";
...
export class TiktokPostScrapeEntry extends ScrapeEntry<TiktokPostMetadata> {
    ...
    async chunk() {
        let docs: Document[] = []
        for (let text of (await TiktokPostScrapeEntry.splitter.splitText(this.body))) {
            docs.push(
                new Document({
