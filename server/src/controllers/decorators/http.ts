import { HttpVerb } from 'src/shared/constants/http-verbs.js';
...
export function Get(path: string) {
    return (target: any, propertyKey: string): void => {
        ...
        const dict = Reflect.getMetadata('routes', target.constructor) as {
            [key: string]: RouteInput;
        };
        if (dict[methodId]) {
            throw new Error(
                `Decorator method \"${methodId}\" is already mapped to a route`,
            );
        }
        dict[methodId] = {
